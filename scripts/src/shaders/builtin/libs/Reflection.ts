...
namespace MITOIA.BuiltinShader.Lib.Reflection {
    ...
    /**
     * @param normal (vec3).
     * @param viewDir (vec3) vertexPos - eyePos.
...
#include<${General.DECLARE_UNIFORM.name}>(vec3, ${ShaderPredefined.v_PosW})
...
#include<${General.DECLARE_UNIFORM.name}>(samplerCube, ${ShaderPredefined.s_ReflectionSampler})
...
vec4 _reflectColor = texture2D(${ShaderPredefined.s_ReflectionSampler}, ${CUBIC_FUNC.name}(${ShaderPredefined.var_ViewDirW}, ${ShaderPredefined.var_NormalW}));
