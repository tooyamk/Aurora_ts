    ...
    export abstract class Merge {
        ...
        public static sort<T>(L: T[], compareFn: (a: T, b: T) => boolean, start: number = 0, end: number = -1) {
            ...
            if (start < end) {
                ...
                let k = start + 1;
                while (k < end) {
                    Merge._mergePass<T>(L, TR, k, L.length, compareFn);
                    k <<= 1;
                    Merge._mergePass<T>(TR, L, k, L.length, compareFn);
                    k <<= 1;
                }
            }
        }

        private static _mergePass<T>(SR: T[], TR: T[], s: number, n: number, compareFn: (a: T, b: T) => boolean) {
            let i = 0, j: number, s2 = s << 1, s_1 = s - 1;
        ...
        private static _merge<T>(SR: T[], TR: T[], i: number, m: number, n: number, compareFn: (a: T, b: T) => boolean) {
