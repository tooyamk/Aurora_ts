...
#define ${NEED_SKINNING_MACRO}

#include<${General.DECLARE_UNIFORM_ARRAY_MACRO.name}>(vec4, ${ShaderPredefined.u_SkinningMatrices}, ${ShaderPredefined.MAX_BONES} * 3)
...
void ${CALC_SKINNING_MATRIX_FUNC}_Single(inout ${SKINNED_MATRIX_STRUCT} mat, float index, float weight) {
    ...
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(float, ${ShaderPredefined.a_BoneIndex0})
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(float, ${ShaderPredefined.a_BoneWeight0})
    ...
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec2, ${ShaderPredefined.a_BoneIndex0})
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec2, ${ShaderPredefined.a_BoneWeight0})
    ...
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec3, ${ShaderPredefined.a_BoneIndex0})
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec3, ${ShaderPredefined.a_BoneWeight0})

    void ${CALC_SKINNING_MATRIX_FUNC}(inout ${SKINNED_MATRIX_STRUCT} mat, vec3 indices, vec3 weights) {
#else
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec4, ${ShaderPredefined.a_BoneIndex0})
    #include<${General.DECLARE_ATTRIB_MACRO.name}>(vec4, ${ShaderPredefined.a_BoneWeight0})
