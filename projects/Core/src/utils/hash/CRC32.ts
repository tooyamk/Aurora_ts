namespace Aurora.CRC32 {
    ...
    export function calc(bytes: Uint8Array, offset: uint, length: int = -1): uint {
        let crc = 0xFFFFFFFF;

        let len = bytes.length;
        if (offset < len) {
            if (length < 0 || offset + length > len) length = len - offset;

            for (let i = offset, n = offset + length; i < n; ++i) crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ bytes[i]) & 0xFF];
    ...
    /**
     * @param crc begin = 0xFFFFFFFF.
     */
    export function calcStep(crc: uint, value: ubyte): uint {
        crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ value) & 0xFF];
        if (crc < 0) crc += 4294967296;
        return crc;
    }

    export function calcFinish(crc: uint): uint {
        crc ^= 0xFFFFFFFF;
        if (crc < 0) crc += 4294967296;
        return crc;
    }

    export function calcUintStep(crc: uint, value: uint): uint {
        crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ (value & 0xFF)) & 0xFF];
        crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ (value >> 8 & 0xFF)) & 0xFF];
        crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ (value >> 16 & 0xFF)) & 0xFF];
        crc = ((crc >> 8) & 0x00FFFFFF) ^ TABLE[(crc ^ (value >> 24 & 0xFF)) & 0xFF];
        if (crc < 0) crc += 4294967296;
        return crc;
    }
}