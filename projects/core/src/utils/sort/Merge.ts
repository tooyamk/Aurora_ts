namespace Aurora.Sort.Merge {
     ...
     */
    export function sort<T>(L: T[], compareFn: (a: T, b: T) => boolean, start: int = 0, end: int = -1) {
    ...
    function _mergePass<T>(SR: T[], TR: T[], s: int, n: int, compareFn: (a: T, b: T) => boolean) {
        let i = 0;
    ...
    function _mergePassOffset<T>(SR: T[], SROffset: int, TR: T[], TROffset:int, s: int, n: int, compareFn: (a: T, b: T) => boolean) {
        let i = 0;
        const s2 = s << 1, s_1 = s - 1;
        const nn = n - s2, s2_1 = s2 - 1;
        while (i <= nn) {
    ...
    function _merge<T>(SR: T[], TR: T[], i: int, m: int, n: int, compareFn: (a: T, b: T) => boolean) {
        let j: int, k: int;
