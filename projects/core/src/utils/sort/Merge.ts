namespace Aurora.Sort {
    export abstract class Merge {
         ...
         */
        public static sort<T>(L: T[], compareFn: (a: T, b: T) => boolean, start: int = 0, end: int = -1) {
            ...
            if (start < end) {
                ...
                if (start === 0) {
                    while (k < end) {
                        Merge._mergePass<T>(L, TR, k, len, compareFn);
                        k <<= 1;
                        Merge._mergePass<T>(TR, L, k, len, compareFn);
                        k <<= 1;
                    }
                } else {
                    while (k < end) {
                        Merge._mergePassOffset<T>(L, start, TR, 0, k, len, compareFn);
                        k <<= 1;
                        Merge._mergePassOffset<T>(TR, 0, L, start, k, len, compareFn);
                        k <<= 1;
                    }
                }
            }
        ...
        private static _merge<T>(SR: T[], TR: T[], i: int, m: int, n: int, compareFn: (a: T, b: T) => boolean) {
            let j: int, k: int;
        ...
        private static _mergeOffset<T>(SR: T[], SROffset: int, TR: T[], TROffset: int, i: int, m: int, n: int, compareFn: (a: T, b: T) => boolean) {
            let j: int, k: int;
            for (j = m + 1, k = i; i <= m && j <= n; ++k) {
                if (compareFn(SR[i + SROffset], SR[j + SROffset]))
                    TR[k + TROffset] = SR[SROffset + i++];
                else
                    TR[k + TROffset] = SR[SROffset + j++];
            }
            while (i <= m) TR[TROffset + k++] = SR[SROffset + i++];
            while (j <= n) TR[TROffset + k++] = SR[SROffset + j++];
        }
    }
}